...
class Node():
    ...
    def handleBlock(self,block):
        ...
        transansactionValid = self.blockchain.transactionsValid(block.transactions)
        signatureValid = Wallet.signatureValid(blockHash,signature,forger)
        if not BlockCountValid:
            self.requestChain()
        
        if lastBlockHashValid and forgerValid and transansactionValid and signatureValid and BlockCountValid :
